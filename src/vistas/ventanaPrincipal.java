...
public class ventanaPrincipal extends JFrame implements KeyListener {
	...
	private void iniciarComponentes() {
		setIconImage(
				Toolkit.getDefaultToolkit().getImage(getClass().getResource("/recursos/imagenes/logoVentana.png")));
		setResizable(false);
	...
	private void consultarArtiulo(String codigo) {
		...
		if (miConexion != null) {
			if (!codigo.isEmpty()) {
				try {
					ArticuloVo miArticulo = miCoordinador.consultarArticulo(codigo, miConexion);
	...
	private void imprimir(ArticuloVo miArticulo) {
		...
		if (printService.length > 0) {
			...
			if (impresora != null) {
				...
				try {
					JasperReport report = JasperCompileManager.compileReport(dir_current + "/reportes/etiquetas.jrxml");// (JasperReport)
																														// JRLoader.loadObject(ruta);
